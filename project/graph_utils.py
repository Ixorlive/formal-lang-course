...
def get_graph_info(graph: MultiGraph) -> GraphInfo:
    ...
    """
    edge_labels = set(label for _, _, label in graph.edges(data="label") if label)  # Extract non-empty edge labels
    return GraphInfo(
...
def create_labeled_two_cycles_graph(n: Union[int, Iterable[Any]],
                                    m: Union[int, Iterable[Any]],
                                    labels: Tuple[str, str]) -> MultiGraph:
    """
...
def create_and_save_labeled_two_cycles_graph(n: Union[int, Iterable[Any]],
                                             m: Union[int, Iterable[Any]],
